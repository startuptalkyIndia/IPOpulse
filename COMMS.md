...
## 2026-07-14 — audit remediation wave 2: ~20 HIGH + ~20 MEDIUM findings (12 commits)

SDD-lite: **What** — worked the enterprise-audit backlog after the CRIT fixes. HIGHs: broker tokens AES-encrypted at rest (encrypt.ts + ENCRYPTION_KEY on server); banned users blocked at login; promoter-check prompt hardened (defamation/SEBI); ICS `?email=` data-leak removed; GMP tracker preserves admin-curated entries + UTC date-key; alerts marked fired only on real delivery (+attempt_count column, capped retry); heartbeat gained a dead-man ping + per-source (NSE/BSE) freshness + 6 more job-liveness checks; dark-mode `[class*=]`→`[class~=]` fixes washed-out bars; IPO status single date-authority (mapStatus keyword-only); status regression tests; CI turned into a real gate. MEDIUMs: shared ipo-name-match + canonical recalcMarketCap (dedup); feedback + push rate-limiting; runIngestion single-flight + fail-on-all-errors; tablet-nav dead zone; full .env.example; admin token pages server-guarded; canonical listing-gain price; deleted dead duplicate proxy.ts; awaited AI spend logging; no raw-error leaks from AI routes; withRetry on all NSE fetches; mobile-drawer dialog a11y. **Why** — production-readiness audit (see IPOpulse Enterprise Audit artifact). **Done when** — tsc clean, 97 unit tests green, each batch type-checked locally. **Out of scope (deferred w/ reason)** — ISR/caching + scalability infra (need live verification), SEBI advice removal (M6/M9 = legal call), snake_case column rename (M1 destructive), 32 low-severity polish. **DB change** — one additive column `alerts.attempt_count` (applied via explicit ALTER; dry-run confirmed no destructive diff). **Verify** — deploy + smoke every changed surface.
